...
public class WidgetController {
    ...
    @RequestMapping(value = "/api/widget/update/{id}/{name}/{type}", method = RequestMethod.GET, produces = "application/json")
    @JsonView(com.example.entity.Widget.class)
    public Widget updateWidget(@PathVariable("id") Long id, @PathVariable("name") String name, @PathVariable("type") String type) {
        Widget widget = widgetRepository.findOne(id);
        if (widget == null) {
            throw new NotFoundException(id.toString());
        }
